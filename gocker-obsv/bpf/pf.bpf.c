...
// 說明：pf 模組負責統計各 cgroup 的 user/kernel 缺頁事件，透過 cfg_map 熱更新白名單與取樣設定
...
// 說明：load_cfg() 會優先採用 user space 下發的 config，未設定時回退常數
...
// 說明：in_target_subtree_current() 透過 ancestor cgroup id 判斷事件是否屬於目標子樹
...
// 說明：pass_sample() 實作取樣率控制，避免事件量過大
